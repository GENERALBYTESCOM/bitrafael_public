        ...
        if ((input.startsWith("L") || input.startsWith("3") || input.startsWith("M")) &&  input.length() <= 34) {
            ...
            try {
                if (isAddressValidInternal(input)) {
                    return new Classification(Classification.TYPE_ADDRESS,IClient.LTC,input,containsPrefix,prefix);
                }
            } catch (AddressFormatException e) {
                e.printStackTrace();
            }
            ...
            try {
                ...
                return new Classification(Classification.TYPE_PRIVATE_KEY_IN_WIF,IClient.LTC,input,containsPrefix,prefix);
            } catch (AddressFormatException e) {
                e.printStackTrace();
            }
        }else if (input.startsWith("Ltub")) {
            return new Classification(Classification.TYPE_XPUB,IClient.LTC,input,containsPrefix,prefix);
        }
